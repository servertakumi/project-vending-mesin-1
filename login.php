...
if (isset($_POST['login'])){
  ...
  $checkusername = mysqli_query($conn,"SELECT * FROM admin WHERE username ='$username'");
  if (mysqli_num_rows($checkusername) > 0){
    $admin = mysqli_fetch_assoc($checkusername);
    $chekpassword = password_verify($password,$admin['password']);
      if ($chekpassword){
        $_SESSION['admin'] = $admin;
        mysqli_query($conn,"UPDATE admin SET logged_in_at = NOW() WHERE username = '$username'");
        header('Location: app?page=profil');
        exit;
      }else {
    ...
    <div class="container">
      ...
      <div>
        <ul class="navbar-nav d-lg-block d-none">
          <li class="nav-item">
            <a href="index?page" class="btn btn-sm  bg-gradient-primary  btn-round mb-0 me-1" onclick="smoothToPricing('pricing-soft-ui')">Kembali</a>
          </li>
        </ul>
      </div>
    </div>
      ...
      <div class="container">
        <div class="row justify-content-center">
          <div class="col-lg-5 text-center mx-auto">
            ...
            <p class="text-lead text-white">Mulailah Belanja Lewat Masa Depan</p>
          </div>
        </div>
      </div>
    </div>
          ...
          <div class="card z-index-0">
            ...
            <div class="card-body">
              <form class="pt-3" action="" method="POST">
                ...
                <p class="text-sm mt-3 mb-0">Already have an account? <a href="javascript:;" class="text-dark font-weight-bolder">register</a></p>
              </form>
            </div>
          </div>
        </div>
